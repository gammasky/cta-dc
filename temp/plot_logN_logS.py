 ...
 #   for i in range(1, num_bins):
  ...
  #            np.log10(y_hgps[i]))  # , ' ', bin_edges_hgps[num_bins - i], '  ',(y_hgps[i]))
   ...
   ## logy = np.log10(y_hgps)
    ...
    plt.plot([8, 8], [flux_min, 100], 'k-', lw=1, color='black')
...
def remove_bright_pwn(table_sim_pwn):
    ...
    table_sim_pwn_reduced = table_sim_pwn['source_name','spec_norm_crab']
...
if __name__ == '__main__':
    ...
    print('-----------------------------------------------------')
    table_pwn_reduced = table_pwn['Source_Name','flux_crab']
