...
class GPSSkyModel:
   ...
   #     return dict(tag=tag, filename=filename, table=table, color='orange')
    ...
    def plot_glon(self):
        ...
        for component in self.get_components(tags=['snr', 'composite','pwn']):#skip=['image_sources']):
    ...
    def plot_glat(self):
        ...
        for component in self.get_components(tags=['pwn', 'composite','snr']):#skip=['image_sources']):
    ...
    def plot_n0(self):
        ...
        for component in self.get_components(tags=['snr']):
            ...
            for row in table:
                vals.append(row['n0'])
                r.append(row['galactocentric_r'])
            ax.hist(
                vals, bins=bins, label=component['tag'], histtype='step',
                alpha=0.8, normed=True,
            )
            ax_2.scatter(r, vals, label=component['tag'], s=10, alpha=0.8, color=component['color'])

        ax_2.legend(loc='best')
        # ax.set_xlim(bins[0], bins-[1])
        # ax.set_xlim(0, 2)
        ax_2.set_xlabel('n0 (1/cm^3)')
        fig_2.tight_layout()
        filename = 'ctadc_skymodel_gps_snrs_n0.png'
        log.info('Writing {}'.format(filename))
    ...
    def get_logn_logs(self, quantity, variant, tags):
        #print('ere')
        bins = np.logspace(-2.1, 2.1, 1000)
        #bins = np.logspace(-0.1, 1.0, 1000)
        crab_flux_above_1TeV, crab_flux_above_1TeV_model = define_flux_crab_above_energy();
        crab_1_10 = crab_flux_above_1TeV.value
        hists = []
        hists_skip = []
        ...
        for component in self.get_components(tags=tags):
            ...
            if ( tag == 'gammacat'):
                ...
                fluxes_skip = []
                for row in table:
                    flux = (row['int_flux_above_1TeV']/crab_1_10)*100
    ...
    def plot_logn_logs(self, quantity, variant, sigma, tags=['gammacat', 'pwn', 'composite', 'snr' ]):
        ...
        hists_skip_mine, hists_mine = self.get_logn_logs(quantity, variant, tags=['composite', 'pwn','gammacat', 'snr'])
        hists_skip_pwn_snr, hists_pwn_snr = self.get_logn_logs(quantity, variant, tags=['composite', 'pwn','snr'])
        ...
        #hists_skip_mine.total.smooth(sigma=sigma).plot_smooth(
        #    ax, alpha=0.8, lw=2, color='magenta', linestyle='dashed', label='mine' )

        #hists_skip.total.smooth(sigma=sigma).plot_smooth(ax, label='total', alpha=0.8, lw=1, color='orange')
        hists_skip_pwn_snr.total.smooth(sigma=sigma).plot_smooth(
            ax, alpha=0.8, lw=2, color='black', linestyle='dashed', )
        hists_pwn_snr.total.smooth(sigma=sigma).plot_smooth(
            ax, label='pwn+composite+snr', alpha=0.8, lw=2, color='black', )
        idx_skip = 0
        ...
        #ax.set_ylim(0, 200)
        ax.set_xlabel('Integral flux 1-10 TeV (Crab units)')
        if quantity == 'n':
